...
void matrix_update(tdma_matrix_t *newMat, uint8_t other_IP) {
    ...
    /* MÉTODO ANA: o routing recalcula a partir da MST a cada ronda no
     * tx_loop, por isso não há fila de eventos. Apenas registamos. */
    (void)other_IP;
    if(topology_changed)
        printf("[MATRIX] TOPOLOGIA MUDOU — routing actualiza na próxima ronda\n\n");
    else
...
void primAlgorithm_weighted(void) {
    ...
    for(int count = 0; count < num; count++) {
        ...
        in_mst[u] = true;
        for(int v = 0; v < num; v++) {
            /* ── MÉTODO ANA MORAIS (tese 3.1.1): matriz BINÁRIA ──
             * O Prim corre sobre a matriz de conectividade binária,
             * SEM qualquer custo de link quality. Todas as arestas
             * presentes (fwd || rev) têm o mesmo peso (1). Como a tese
             * assume, a MST resultante depende apenas da ordem dos nós
             * e da sequência de execução do algoritmo.
             * (O OR entre fwd/rev mantém-se para tolerar staleness de
             *  até 1 frame na actualização da matriz do vizinho.) */
