    ...
    while (node->running) {
        ...
        switch (evt->type) {
            ...
            case EVENT_TOPOLOGY_CHANGED: {
                ...
                /*
                 * Recalcula rotas em user-space E escreve no kernel numa
                 * só chamada. Para cada destino activo, dentro do loop:
                 *   1. lookup_next_hop() → next_hop
                 *   2. preenche routing_table[i]
                 *   3. ip_route_add("10.0.0.<dest>", "10.0.0.<next_hop>", "wlan0")
