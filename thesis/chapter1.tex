...
all inter-node communication, which is well suited to the periodic topology
beacons but provides no retransmission mechanism for video frames, causing lost
datagrams to produce visible artefacts at the receiver. Prior work demonstrated
that adopting TCP at the application layer eliminates these artefacts, but that
using TCP with default protocol settings introduces buffering delays that are
incompatible with real-time streaming over a TDMA network. Properly integrating
TCP into the communication stack requires the forwarding path to be visible at
the IP layer, since TCP connections are identified by endpoint addresses and
port numbers. This interaction between transport protocol and forwarding
