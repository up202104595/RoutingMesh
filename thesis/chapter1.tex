...
developed at FEUP~\cite{diogo2023} extends this model to ad-hoc mesh networks
with relative synchronisation --- that is, without requiring a global clock
--- and distributes topology information via periodic beacon packets. An
experimental implementation of RA-TDMAs+ was realised by Diogo
Almeida~\cite{diogo2023}, providing a functional TDMA framework as the
foundation upon which the present work is built.
...
transit packets must be implemented efficiently: a user-space forwarding plane
would introduce additional latency and complexity, while kernel-level IP
forwarding offers proven performance but requires careful integration with the
TDMA node's routing logic. Fourth, a metric is needed to quantify link quality
...
This dissertation addresses these problems in the context of a three-node
scenario: a mobile robot acting as the video source, an intermediate relay
node, and a base station where the operator receives the video stream and sends
control commands. This minimal topology is sufficient to exercise the essential
features of multi-hop routing --- direct link communication, link failure,
relay activation, and recovery --- while keeping experimental conditions
...
\begin{itemize}
    ...
    \item \textbf{Experimental evaluation}: a test methodology and set of
    measurement scripts for characterising routing convergence time, round-trip
    latency, video stream inter-arrival regularity, and throughput in a
    three-node scenario with topology changes induced by controlled signal
    attenuation, producing comparative results for direct and relay
...
\begin{itemize}
    ...
    related work on kernel-level routing integration in wireless mesh networks.
