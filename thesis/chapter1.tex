...
all inter-node communication, which is well suited to periodic topology beacons
but provides no retransmission mechanism for video frames, causing lost datagrams
to appear as visible artefacts at the receiver. Adopting TCP as the data
transport addresses this problem, but it requires the forwarding path to be
visible at the IP layer, since TCP connections are identified by IP endpoint
addresses and port numbers. This interaction between transport protocol and
forwarding architecture is a central design constraint of the present work.
